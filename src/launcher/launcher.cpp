...
#define DEFAULT_HL2_GAMEDIR	"hl2or"
...
void CLogAllFiles::Init()
{
	...
	char path[MAX_PATH];
	Q_snprintf( path, sizeof(path), "%s/%s", GetBaseDirectory(), CommandLine()->ParmValue( "-game", "hl2or" ) );
	Q_FixSlashes( path );
#ifdef WIN32
	Q_strlower( path );
#endif
...
bool CSourceAppSystemGroup::PreInit()
{
	...
	if ( IsPC() )
	{
		...
		reslistgenerator->Init( GetBaseDirectory(), CommandLine()->ParmValue( "-game", "hl2or" ) );
