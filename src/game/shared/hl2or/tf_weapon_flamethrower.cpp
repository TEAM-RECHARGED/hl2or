...
LINK_ENTITY_TO_CLASS( weapon_flamethrower, CTFFlameThrower );
...
void CTFFlameThrower::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if ( IsCarrierAlive() && ( WeaponState() == WEAPON_IS_ACTIVE ) && ( GetOwner()->GetAmmoCount( m_iPrimaryAmmoType ) > 0 ) )
	{
		if ( m_iWeaponState > FT_STATE_IDLE )
		{
			StartFlame();
...
void CTFFlameEntity::ImmolationDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore)
{
...
void CTFFlameEntity::FlameThink( void )
{
	...
	// Clamp
	float m_flBurnRadius = MIN( 64.0f, 256.0f );
	...
	if ( GetAbsOrigin() != m_vecPrevPos )
	{
		...
		if (pEntity)
		{
			CheckCollision(pEntity, false);
		}

		CTakeDamageInfo( this, this, 1, DMG_BURN );

		CBasePlayer *pAttacker = dynamic_cast<CBasePlayer *>( (CBaseEntity *) m_hAttacker );
		if ( !pAttacker )
			return;

		CBaseEntity *pTeam = pAttacker->GetEnemy();
		if ( !pTeam )
			return;
		...
		if ( pTeam )
		{
			CheckCollision( pTeam, &bHitWorld );
			if ( bHitWorld )
				return;
		}
