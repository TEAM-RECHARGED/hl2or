...
ConVar	sk_adrenaline("sk_adrenaline", "0");
ConVar	sk_adrenvial("sk_adrenvial", "0");
ConVar	sk_adrencharger("sk_adrencharger", "0");
...
{
public:
	DECLARE_CLASS(Cadrenaline, CItem);
...
PRECACHE_REGISTER(item_adrenaline);


//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void Cadrenaline::Spawn(void)
{
	Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void Cadrenaline::Precache(void)
{
	PrecacheModel("models/w_medkit.mdl");

	PrecacheScriptSound("adrenaline.Touch");
}
...
{
	if (pPlayer->TakeHealth(sk_adrenaline.GetFloat(), DMG_GENERIC))
	{
		...
		CPASAttenuationFilter filter(pPlayer, "adrenaline.Touch");
		EmitSound(filter, pPlayer->entindex(), "adrenaline.Touch");
...
public:
	...
	bool MyTouch(CBasePlayer* pPlayer)
	{
		if (pPlayer->TakeHealth(sk_adrenvial.GetFloat(), DMG_GENERIC))
		{
			CSingleUserRecipientFilter user(pPlayer);
			user.MakeReliable();
...
void CWallHealth::Spawn(void)
{
	...
	m_iJuice = sk_adrencharger.GetFloat();
