...
// Purpose: 
//
// $NoKeywords: $AddedByGuestSneezePlayZ
//=============================================================================//
...
//Debug visualization
ConVar	g_debug_lighter_xen("g_debug_turret_xen", "0");

#define	XEN_LIGHTER_MODEL		"models/xenlighter.mdl"
#define XEN_LIGHTER_GLOW_SPRITE	"sprites/glow1.vmt"
#define	XEN_LIGHTER_RANGE		1300
#define XEN_LIGHTER_SPREAD		VECTOR_CONE_2DEGREES
#define	XEN_LIGHTER_MAX_WAIT		3
#define	XEN_LIGHTER_PING_TIME	1.0f	//LPB!!

#define	XEN_LIGHTER_VOICE_PITCH_LOW	45
#define	XEN_LIGHTER_VOICE_PITCH_HIGH	100

//Aiming variables
#define	XEN_LIGHTER_MAX_NOHARM_PERIOD	0.0f
#define	XEN_LIGHTER_MAX_GRACE_PERIOD		3.0f

//Spawnflags
#define SPAWNFLAG_XEN_LIGHTER_AUTOACTIVATE		0x00000020
#define SPAWNFLAG_XEN_LIGHTER_STARTINACTIVE		0x00000040
#define SPAWNFLAG_XEN_LIGHTER_NEVERRETIRE		0x00000080
#define SPAWNFLAG_XEN_LIGHTER_OUT_OF_AMMO		0x00000100
...
int ACT_LIGHTER_CLOSE;
int ACT_LIGHTER_OPEN_IDLE;
int ACT_LIGHTER_CLOSED_IDLE;
int ACT_LIGHTER_FIRE;
int ACT_LIGHTER_DRYFIRE;

//Xen Lighter States
enum xenState_e
{
	LIGHTER_SEARCHING,
	LIGHTER_AUTO_SEARCHING,
	LIGHTER_ACTIVE,
	LIGHTER_DEPLOYING,
	LIGHTER_RETIRING,
	LIGHTER_DEAD,
...
enum eyeState_t
{
	LIGHTER_EYE_SEE_TARGET,			//Sees the target, bright and big
	LIGHTER_EYE_SEEKING_TARGET,		//Looking for a target, blinking (bright)
	LIGHTER_EYE_DORMANT,				//Not active
	LIGHTER_EYE_DEAD,				//Completely invisible
	LIGHTER_EYE_DISABLED,			//Turned off, must be reactivated before it'll deploy again (completely invisible)
};

class CNPC_LIGHTER : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_LIGHTER, CAI_BaseNPC);
public:
...
protected:
	...
	bool	PreThink(xenState_e state);
...
LINK_ENTITY_TO_CLASS(npc_xenlighter, CNPC_LIGHTER);
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Precache(void)
{
	PrecacheModel(XEN_LIGHTER_MODEL);
	PrecacheModel(XEN_LIGHTER_GLOW_SPRITE);

	// Activities
	ADD_CUSTOM_ACTIVITY(CNPC_LIGHTER, ACT_LIGHTER_OPEN);
	ADD_CUSTOM_ACTIVITY(CNPC_LIGHTER, ACT_LIGHTER_CLOSE);
	ADD_CUSTOM_ACTIVITY(CNPC_LIGHTER, ACT_LIGHTER_CLOSED_IDLE);
	ADD_CUSTOM_ACTIVITY(CNPC_LIGHTER, ACT_LIGHTER_OPEN_IDLE);
	ADD_CUSTOM_ACTIVITY(CNPC_LIGHTER, ACT_LIGHTER_FIRE);
	ADD_CUSTOM_ACTIVITY(CNPC_LIGHTER, ACT_LIGHTER_DRYFIRE);

	// TODO: Create new sounds
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Spawn(void)
{
	Precache();

	SetModel(XEN_LIGHTER_MODEL);
	...
	SetHeight(XEN_LIGHTER_RETRACT_HEIGHT);
	...
	m_pEyeGlow = CSprite::SpriteCreate(XEN_LIGHTER_GLOW_SPRITE, GetLocalOrigin(), false);
	...
	m_bAutoStart = !!(m_spawnflags & SPAWNFLAG_XEN_LIGHTER_AUTOACTIVATE);
	m_bEnabled = ((m_spawnflags & SPAWNFLAG_XEN_LIGHTER_STARTINACTIVE) == false);

	//Do we start active?
	if (m_bAutoStart && m_bEnabled)
	{
		SetThink(&CNPC_LIGHTER::AutoSearchThink);
		SetEyeState(LIGHTER_EYE_DORMANT);
	}
	else
	{
		SetEyeState(LIGHTER_EYE_DISABLED);
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_LIGHTER::OnTakeDamage(const CTakeDamageInfo& inputInfo)
{
	if (!m_takedamage)
		return 0;
	...
	if (m_iHealth <= 0)
	{
		...
		SetThink(&CNPC_LIGHTER::DeathThink);
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Retire(void)
{
	if (PreThink(LIGHTER_RETIRING))
		return;
	...
	if (GetActivity() != ACT_LIGHTER_CLOSE)
	{
		//Set our visible state to dormant
		SetEyeState(LIGHTER_EYE_DORMANT);

		SetActivity((Activity)ACT_LIGHTER_OPEN_IDLE);

		//If we're done moving to our desired facing, close up
		if (UpdateFacing() == false)
		{
			SetActivity((Activity)ACT_LIGHTER_CLOSE);
	...
	else if (IsActivityFinished())
	{
		...
		SetActivity((Activity)ACT_LIGHTER_CLOSED_IDLE);
		...
		else
		{
			//Set our visible state to dormant
			SetEyeState(LIGHTER_EYE_DISABLED);
			SetThink(&CNPC_LIGHTER::SUB_DoNothing);
		}
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Deploy(void)
{
	if (PreThink(LIGHTER_DEPLOYING))
		return;
	...
	SetEyeState(LIGHTER_EYE_SEE_TARGET);

	//Open if we're not already
	if (GetActivity() != ACT_LIGHTER_OPEN)
	{
		m_bActive = true;
		SetActivity((Activity)ACT_LIGHTER_OPEN);
	...
	{
		SetHeight(XEN_LIGHTER_DEPLOY_HEIGHT);
		...
		m_flPlaybackRate = 0;
		SetThink(&CNPC_LIGHTER::SearchThink);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::SetLastSightTime()
{
	if (HasSpawnFlags(SPAWNFLAG_XEN_LIGHTER_NEVERRETIRE))
	{
		m_flLastSight = FLT_MAX;
	}
	else
	{
		m_flLastSight = gpGlobals->curtime + XEN_LIGHTER_MAX_WAIT;
	}
}
...
//-----------------------------------------------------------------------------
float CNPC_LIGHTER::MaxYawSpeed(void)
{
...
//-----------------------------------------------------------------------------
bool CNPC_LIGHTER::UpdateFacing(void)
{
	bool  bMoved = false;
	...
	if (g_debug_lighter_xen.GetBool())
	{
...
//-----------------------------------------------------------------------------
bool CNPC_LIGHTER::FVisible(CBaseEntity* pEntity, int traceMask, CBaseEntity** ppBlocker)
{
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::ActiveThink(void)
{
	//Allow descended classes a chance to do something before the think function
	if (PreThink(LIGHTER_ACTIVE))
		return;
	...
	if ((bEnemyVisible == false) || (flDistToEnemy > XEN_LIGHTER_RANGE))
	{
		if (m_flLastSight)
		{
	...
	if (m_flShotTime < gpGlobals->curtime)
	{
		...
		{
			if (m_spawnflags & SPAWNFLAG_XEN_LIGHTER_OUT_OF_AMMO)
			{
				SetActivity((Activity)ACT_LIGHTER_DRYFIRE);
			}
			else
			{
				SetActivity((Activity)ACT_LIGHTER_FIRE);
			}
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::SearchThink(void)
{
	//Allow descended classes a chance to do something before the think function
	if (PreThink(LIGHTER_SEARCHING))
		return;
	...
	if (GetEnemy() == NULL)
	{
		GetSenses()->Look(XEN_LIGHTER_RANGE);
	...
	if (GetEnemy() != NULL)
	{
		...
		m_flLastSight = 0;
		SetThink(&CNPC_LIGHTER::ActiveThink);
		SetEyeState(LIGHTER_EYE_SEE_TARGET);
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::AutoSearchThink(void)
{
	//Allow descended classes a chance to do something before the think function
	if (PreThink(LIGHTER_AUTO_SEARCHING))
		return;
...
// Purpose: Fire!
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Shoot(const Vector& vecSrc, const Vector& vecDirToEnemy)
{
	if (m_spawnflags & SPAWNFLAG_XEN_LIGHTER_OUT_OF_AMMO)
	{
...
//-----------------------------------------------------------------------------
bool CNPC_LIGHTER::PreThink(xenState_e state)
{
	CheckPVSCondition();
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::SetEyeState(eyeState_t state)
{
	...
	{
	default:
	case LIGHTER_EYE_SEE_TARGET: //Fade in and scale up
		m_pEyeGlow->SetColor(255, 0, 0);
		m_pEyeGlow->SetBrightness(164, 0.1f);
		m_pEyeGlow->SetScale(0.4f, 0.1f);
		break;

	case LIGHTER_EYE_SEEKING_TARGET: //Ping-pongs
...
void CNPC_LIGHTER::Ping(void)
{
	...
	SetEyeState(LIGHTER_EYE_SEEKING_TARGET);

	m_flPingTime = gpGlobals->curtime + XEN_LIGHTER_PING_TIME;
}

//-----------------------------------------------------------------------------
// Purpose: Toggle the turret's state
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Toggle(void)
{
	//Toggle the state
	if (m_bEnabled)
	{
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::Disable(void)
{
	m_bEnabled = false;
	m_bAutoStart = false;
...
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::InputToggle(inputdata_t& inputdata)
{
	Toggle();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::InputEnable(inputdata_t& inputdata)
{
	Enable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::InputDisable(inputdata_t& inputdata)
{
	Disable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::SpinUp(void)
{
}

#define	XEN_LIGHTER_MIN_SPIN_DOWN	1.0f

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::SpinDown(void)
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::DeathThink(void)
{
	if (PreThink(LIGHTER_DEAD))
		return;
...
// Input  : height - 
//-----------------------------------------------------------------------------
void CNPC_LIGHTER::SetHeight(float height)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_LIGHTER::CanBeAnEnemyOf(CBaseEntity* pEnemy)
{
