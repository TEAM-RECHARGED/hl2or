...
void CWeaponGaussGun::ItemPostFrame(void)
{
	...
	// Handle charge state and button release (secondary fire)
	...
	// Update spinning bits (coil and fan)
	SetBoneController(0, fanAngle);
	SetBoneController(1, m_flCoilAngle);

	// Handle inspection animation
	if ((pPlayer->m_afButtonPressed & IN_RELOAD) && m_flNextFidgetReload < gpGlobals->curtime)
	{
		// Randomly choose between two inspect animations
		...
		{
			SendWeaponAnim(ACT_VM_INSPECT_ALT);
		}

		// Set cooldown to prevent spam of inspection animation
		m_flNextFidgetReload = gpGlobals->curtime + 2.0f;  // Adjust delay as needed
		return;  // Prevent further actions in this frame
	}
