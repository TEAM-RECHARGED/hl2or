...
ConVar	sk_pitdrone_health("sk_pitdrone_health", "50");
ConVar	sk_pitdrone_dmg_claw("sk_pitdrone_dmg_claw", "8");
ConVar	sk_pitdrone_dmg_clawrake("sk_pitdrone_dmg_clawrake", "25");
ConVar	sk_pitdrone_dmg_zap("sk_pitdrone_dmg_zap", "15");

LINK_ENTITY_TO_CLASS(npc_pitdrone, CNPC_PITDRONE);

BEGIN_DATADESC(CNPC_PITDRONE)
DEFINE_FIELD(m_iBravery, FIELD_INTEGER),
DEFINE_ARRAY(m_pBeam, FIELD_CLASSPTR, PITDRONE_MAX_BEAMS),
...
enum
{
	SCHED_PITDRONE_ATTACK = LAST_SHARED_SCHEDULE,
};

#define PITDRONE_IGNORE_PLAYER 64
...
void CNPC_PITDRONE::Precache()
{
	...
	PrecacheModel("models/pitdrone.mdl");
...
Disposition_t CNPC_PITDRONE::IRelationType(CBaseEntity* pTarget)
{
	if ((pTarget->IsPlayer()))
	{
		if ((GetSpawnFlags() & PITDRONE_IGNORE_PLAYER) && !HasMemory(bits_MEMORY_PROVOKED))
			return D_NU;
	}
...
void CNPC_PITDRONE::CallForHelp(char* szClassname, float flDist, CBaseEntity* pEnemy, Vector& vecLocation)
{
...
void CNPC_PITDRONE::Event_Killed(const CTakeDamageInfo& info)
{
	ClearBeams();
...
void CNPC_PITDRONE::HandleAnimEvent(animevent_t* pEvent)
{
	...
	{
		// SOUND HERE!
		CBaseEntity* pHurt = CheckTraceHullAttack(40, Vector(-10, -10, -10), Vector(10, 10, 10), sk_pitdrone_dmg_claw.GetFloat(), DMG_SLASH);
		CPASAttenuationFilter filter(this);
		if (pHurt)
		{
	...
	{
		CBaseEntity* pHurt = CheckTraceHullAttack(40, Vector(-10, -10, -10), Vector(10, 10, 10), sk_pitdrone_dmg_clawrake.GetFloat(), DMG_SLASH);
...
void CNPC_PITDRONE::TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
...
//=========================================================
void CNPC_PITDRONE::ArmBeam(int side)
{
	trace_t tr;
	float flDist = 1.0;

	if (m_iBeams >= PITDRONE_MAX_BEAMS)
		return;
...
//=========================================================
void CNPC_PITDRONE::WackBeam(int side, CBaseEntity* pEntity)
{
	Vector vecDest;
...
//=========================================================
void CNPC_PITDRONE::ZapBeam(int side)
{
	...
	{
		CTakeDamageInfo info(this, this, sk_pitdrone_dmg_zap.GetFloat(), DMG_SHOCK);
		CalculateMeleeDamageForce(&info, vecAim, tr.endpos);
...
//=========================================================
void CNPC_PITDRONE::ClearBeams()
{
	for (int i = 0; i < PITDRONE_MAX_BEAMS; i++)
	{
		if (m_pBeam[i])
		{
