...
LINK_ENTITY_TO_CLASS( monstermaker_hl1, CNPCHL1Maker );


//-----------------------------------------------------------------------------
// Purpose: Spawn
//-----------------------------------------------------------------------------
void CNPCHL1Maker::Spawn( void )
{
	SetSolid( SOLID_NONE );
	m_cLiveChildren		= 0;
	Precache();
	...
	if ( m_spawnflags & SF_NPCMAKER_HL1_INF_CHILD )
	{
		m_spawnflags |= SF_NPCMAKER_HL1_FADE;
	}

	//Start on?
	if ( m_bDisabled == false )
	{
		SetThink ( &CNPCHL1Maker::MakerThink );
...
//-----------------------------------------------------------------------------
bool CNPCHL1Maker::CanMakeNPC( void )
{
	...
	if ( ( m_spawnflags & SF_NPCMAKER_HL1_NO_DROP ) == false )
	{
		mins.z = m_flGround;
	}
...
//			been created.
//-----------------------------------------------------------------------------
bool CNPCHL1Maker::IsDepleted()
{
	if ( (m_spawnflags & SF_NPCMAKER_HL1_INF_CHILD) || m_iMaxNumNPCs > 0 )
		return false;
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::Disable( void )
{
	m_bDisabled = true;
	SetThink ( NULL );
}
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::InputSpawnNPC( inputdata_t &inputdata )
{
	MakeNPC();
}
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::InputEnable( inputdata_t &inputdata )
{
	Enable();
}
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::InputDisable( inputdata_t &inputdata )
{
	Disable();
}
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::InputToggle( inputdata_t &inputdata )
{
	Toggle();
}
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::Precache( void )
{
	BaseClass::Precache();
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::MakeNPC( void )
{
	if (!CanMakeNPC())
	{
		return;
	...
	if ( m_spawnflags & SF_NPCMAKER_HL1_FADE )
	{
...
//-----------------------------------------------------------------------------
void CNPCHL1Maker::MakerThink ( void )
{
	SetNextThink( gpGlobals->curtime + m_flSpawnFrequency );
...
// Input  : *pVictim - 
//-----------------------------------------------------------------------------
void CNPCHL1Maker::DeathNotice( CBaseEntity *pVictim )
{
