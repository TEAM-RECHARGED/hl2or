...
/*	if (!( m_spawnflags & SF_WAITFORTRIGGER))
	...
	m_flMaxSpeed = (float)BASECHOPPER_MAX_SPEED_HL1 / 2;
...
void CBaseHL1Helicopter::Spawn( void )
{
	...
	m_fHelicopterFlags &= ~BITS_HELICOPTER_MISSILE_HL1_ON;
	m_fHelicopterFlags &= ~BITS_HELICOPTER_GUN_HL1_ON;
	...
	m_flMaxSpeed = BASECHOPPER_MAX_SPEED_HL1;
	m_flMaxSpeedFiring = BASECHOPPER_MAX_FIRING_SPEED_HL1;
	m_takedamage = DAMAGE_AIM;

	// Don't start up if the level designer has asked the 
	// helicopter to start disabled.
	if ( !(m_spawnflags & SF_AWAITINPUT_HL1) )
	{
		Startup();
...
void CBaseHL1Helicopter::Hunt( void )
{
	...
	if (m_fHelicopterFlags & BITS_HELICOPTER_GUN_HL1_ON)
	{
	...
	if (m_fHelicopterFlags & BITS_HELICOPTER_MISSILE_HL1_ON)
	{
		AimRocketGun();
	}
...
// Output : 
//-----------------------------------------------------------------------------
void CBaseHL1Helicopter::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
void CBaseHL1Helicopter::InputActivate( inputdata_t &inputdata )
{
	if( m_spawnflags & SF_AWAITINPUT_HL1)
	{
		...
		// subsequent calls.
		m_spawnflags &= ~SF_AWAITINPUT_HL1;
	}
}
